...
public class VerifyGetRecordByIdTests extends TestCommon {
	...
	@Test
	public void getRecordByIdReturnsInvalidAtomEntry() throws SAXException, IOException, URISyntaxException {
		...
		mockResponse();
		Document entity = docBuilder.parse(this.getClass().getResourceAsStream("/atom/entry-invalid.xml"));
		try (MockedStatic<ClientUtils> clientUtils = Mockito.mockStatic(ClientUtils.class);
		// MockedStatic<ServiceMetadataUtils> serviceMetadataUtils =
		// Mockito.mockStatic(ServiceMetadataUtils.class)
		) {
			...
			clientUtils
				.when(() -> ClientUtils.buildGetRequest(nullable(URI.class), any(Map.class), any(MediaType.class)))
				.thenReturn(rsp);
		...
		// Mockito.doReturn(entity).when(spy).getResponseEntityAsDocument(any(Response.class),
		// anyString());
		// spy.initCommonFixture(testContext);
		// List<String> idList = new ArrayList<>();
		// idList.add("id-01");
