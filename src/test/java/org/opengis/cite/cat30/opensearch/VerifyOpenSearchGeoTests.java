...
public class VerifyOpenSearchGeoTests extends TestCommon {
	...
	public static void initTestFixture() throws Exception {
		...
		when(suite.getAttribute(SuiteAttribute.DATASET.getName())).thenReturn(dataInfo);
		Document doc = docBuilder.parse(VerifyOpenSearchGeoTests.class.getResourceAsStream("/capabilities/basic.xml"));
	...
	public void boundingBoxQuery_disjoint() throws SAXException, IOException {
		...
		Document osd = docBuilder
			.parse(VerifyOpenSearchGeoTests.class.getResourceAsStream("/opensearch/OpenSearchDescription-valid.xml"));
		when(suite.getAttribute(SuiteAttribute.OPENSEARCH_DESCR.getName())).thenReturn(osd);
		mockResponse();
		Document rspEntity = docBuilder.parse(this.getClass().getResourceAsStream("/rsp/feed-1.xml"));
		...
		try (MockedStatic<ClientUtils> clientUtils = Mockito.mockStatic(ClientUtils.class)) {
			clientUtils
				.when(() -> ClientUtils.buildGetRequest(nullable(URI.class), nullable(Map.class), any(MediaType.class)))
				.thenReturn(rsp);
	...
	public void boundingBoxQuery_RSSResponse() throws SAXException, IOException {
		Document osd = docBuilder
			.parse(VerifyOpenSearchGeoTests.class.getResourceAsStream("/opensearch/OpenSearchDescription-rss.xml"));
		when(suite.getAttribute(SuiteAttribute.OPENSEARCH_DESCR.getName())).thenReturn(osd);
		mockResponse();
		Document rspEntity = docBuilder.parse(this.getClass().getResourceAsStream("/rsp/rss-1.xml"));
		OpenSearchGeoTests spy = Mockito.spy(new OpenSearchGeoTests());
		try (MockedStatic<ClientUtils> clientUtils = Mockito.mockStatic(ClientUtils.class)) {
			clientUtils
				.when(() -> ClientUtils.buildGetRequest(nullable(URI.class), nullable(Map.class), any(MediaType.class)))
