...
public class VerifySpatialUtils {
	...
	public void createEnvelopeFromGeoRSSBox() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/georss/box.xml"));
		...
		assertArrayEquals("Unexpected coords for upper corner.", new double[] { 33.5, -116.2 },
				env.getUpperCorner().getCoordinate(), 0.05);
	}

	@Test(expected = IllegalArgumentException.class)
	public void createEnvelopeFromBoxWithMissingCorner() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/georss/box-missingCorner.xml"));
		Envelope env = SpatialUtils.envelopeFromSimpleGeoRSSBox(doc.getDocumentElement());
	...
	public void createEnvelopeFromBoxWithNonNumericValue() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/georss/box-NaN.xml"));
		Envelope env = SpatialUtils.envelopeFromSimpleGeoRSSBox(doc.getDocumentElement());
		assertNull(env);
	}
	...
	public void createEnvelopeFromGML31Envelope() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/Envelope-GML31.xml"));
		...
		assertEquals("Unexpected srsName", "urn:ogc:def:crs:EPSG::32610", env.getAttribute("srsName"));
		String upperCorner = env.getElementsByTagNameNS(Namespaces.GML, "upperCorner").item(0).getTextContent();
