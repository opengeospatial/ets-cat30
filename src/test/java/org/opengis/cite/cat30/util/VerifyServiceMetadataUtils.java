...
public class VerifyServiceMetadataUtils {
	...
	public void getOpenSearchDescriptionConstraint() throws SAXException, IOException {
		...
		Set<String> values = ServiceMetadataUtils.getConstraintValues(doc, "OpenSearchDescriptionDocument");
		assertEquals("Unexpected number of values.", 1, values.size());
		assertEquals(values.iterator().next(), "http://www.sdisuite.de/terraCatalog/opensearch");
	}

	@Test
	public void getAcceptVersionsParamValues() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/capabilities/basic.xml"));
		Set<String> versions = ServiceMetadataUtils.getParameterValues(doc, CAT3.GET_CAPABILITIES,
				CAT3.ACCEPT_VERSIONS);
		assertEquals("Unexpected number of supported versions.", 2, versions.size());
		assertTrue("Expected '3.0.0' as supported version.", versions.contains("3.0.0"));
	}

	@Test
	public void getOutputSchemaValuesForGetRecordById() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/capabilities/basic.xml"));
		Set<String> schemas = ServiceMetadataUtils.getParameterValues(doc, CAT3.GET_RECORD_BY_ID, CAT3.OUTPUT_SCHEMA);
		assertEquals("Unexpected number of supported versions.", 2, schemas.size());
		assertTrue("Expected Atom as supported outputSchema.", schemas.contains(Namespaces.ATOM));
	}
