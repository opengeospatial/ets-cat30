...
public class Records {
	 ...
	 * @return A list of property values, which may be empty if none are found in the
	 * sample data.
	 */
	public static List<String> findPropertyValues(File file, String xpath) {
		...
		try {
			...
			Logger.getLogger(DatasetInfo.class.getName())
				.log(Level.WARNING, "Failed to evaluate XPath expression: " + xpath, ex);
		}
	 ...
	 * elements.
	 * @return A string containing two or more terms (separated by a space character).
	 */
	public static String findMatchingSearchTerms(File dataFile, QName... elemNames) {
		...
		if (results.size() > 0) {
			...
			for (QName elemName : elemNames) {
				String value = lastNode.axisIterator(Axis.CHILD, new net.sf.saxon.s9api.QName(elemName))
					.next()
					.getStringValue();
	 ...
	 * elements.
	 * @return An immutable XdmValue object containing zero or more matching nodes
	 * (XdmItem).
	 */
	public static XdmValue findRecordsInSampleData(File dataFile, QName... properties) {
		...
		try {
			...
			Logger.getLogger(Records.class.getName())
				.log(Level.WARNING,
						String.format("Failed to evaluate XPath expression against sample data file at %s:\n %s ",
								dataFile.getAbsolutePath(), xpath),
						x);
		}
		return value;
	}
