...
public class ETSAssert {
	...
	public static void assertQualifiedName(Node node, QName qName) {
		...
		String nsName = (null != node.getNamespaceURI()) ? node.getNamespaceURI() : XMLConstants.NULL_NS_URI;
		Assert.assertEquals(nsName, qName.getNamespaceURI(), ErrorMessage.get(ErrorMessageKeys.NAMESPACE_NAME));
	}
	...
	/**
	 * Asserts that all bounding boxes appearing in the search results intersect the given
	 * envelope. The following bounding box representations are recognized:
	 * <ul>
	...
	public static void assertEnvelopeIntersectsBoundingBoxes(final Envelope bbox, final Source results) {
		...
		try {
			boxNodeList = (NodeList) XMLUtils.evaluateXPath(results,
					"//ows:BoundingBox | //ows:WGS84BoundingBox | //georss:box | //gml31:Envelope", nsBindings,
			...
			for (int i = 0; i < recordList.getLength(); i++) {
				...
				try {
					...
					Assert.assertTrue(result.size() > 0,
							ErrorMessage.format(ErrorMessageKeys.XPATH_RESULT, Records.getRecordId(record),
					...
					LOGR.log(Level.FINE, "In {0} found {1} matching fields for ''{2}'':\n{3}", new Object[] {
							Records.getRecordId(record), result.size(), term, XMLUtils.writeXdmValueToString(result) });
				}
				catch (SaxonApiException sae) {
					throw new AssertionError(String.format("Failed to evaluate XPath expression: %s \nReason: %s", expr,
							sae.getMessage()));
				}
			}
		}
