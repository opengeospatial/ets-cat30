...
public class OpenSearchCoreTests extends CommonFixture {
	...
	public void initOpenSearchCoreTestsFixture(ITestContext testContext) {
		...
		if (null == this.openSearchDescr) {
			throw new SkipException(
					"OpenSearch description not found in test context.");
		}
		this.templates = ServiceMetadataUtils
				.getOpenSearchURLTemplates(this.openSearchDescr);
		QName searchTermsParam = new QName(Namespaces.OSD11, "searchTerms");
		this.searchTermsTemplates = OpenSearchTemplateUtils
	...
	public void keywordSearch_emptyResultSet() {
		if (this.searchTermsTemplates.isEmpty()) {
			throw new AssertionError(
					"No URL templates containing {searchTerms} parameter.");
		}
	 ...
	 * word (URL-encoded) that occurs in at least one catalog record. The result
	...
	public void singleKeywordSearch() {
		...
		Map<QName, String> values = new HashMap<>();
		values.put(SEARCH_TERMS_PARAM,
				URIUtils.getPercentEncodedString(searchTerm));
		for (Node template : this.searchTermsTemplates) {
			Element urlElem = (Element) template;
			NodeList records;
			try {
			...
			Document entity = getResponseEntityAsDocument(response, null);
			if (entity.getDocumentElement().getNamespaceURI()
					.equals(Namespaces.ATOM)) {
				URL schemaUrl = getClass().getResource(SCHEMATRON_ATOM);
				ETSAssert.assertSchematronValid(schemaUrl,
	 ...
	 * contains two terms (URL-encoded). The result set must not be empty.
	 */
	@Test(description = "OGC 12-176, Table 6: Text search")
	public void multipleKeywordSearch() {
		...
		params.put(SEARCH_TERMS_PARAM,
				URIUtils.getPercentEncodedString(searchTerms));
		for (Node template : this.searchTermsTemplates) {
			Element urlElem = (Element) template;
			NodeList records;
			try {
	...
	public void sliceResults() {
		...
		List<Node> templatesWithCountParam = OpenSearchTemplateUtils
				.filterURLTemplatesByParam(templates, countParam);
		if (templatesWithCountParam.isEmpty()) {
			throw new AssertionError(
					"No URL templates containing {count} parameter.");
		}
		int count = 4;
		Map<QName, String> params = new HashMap<>();
		...
		for (Node template : templatesWithCountParam) {
			...
			case Namespaces.ATOM:
				...
				Assert.assertNotNull(itemsPerPage, ErrorMessage.format(
						...
						"os:itemsPerPage"));
				Assert.assertEquals(Integer.parseInt(itemsPerPage
						.getTextContent()), count, ErrorMessage.format(
						ErrorMessageKeys.INFOSET_ITEM_VALUE, "os:itemsPerPage"));
				...
				Assert.assertEquals(Integer.parseInt(startIndexNode
						.getTextContent()), startIndex, ErrorMessage.format(
						ErrorMessageKeys.INFOSET_ITEM_VALUE, "os:startIndex"));
				break;
			case Namespaces.CSW:
				Node resultsNode = entity.getElementsByTagNameNS(
						Namespaces.CSW, "SearchResults").item(0);
				Assert.assertNotNull(resultsNode, ErrorMessage.format(
						ErrorMessageKeys.MISSING_INFOSET_ITEM,
						"csw:SearchResults"));
				Element results = (Element) resultsNode;
				int nRecordsReturned = Integer.parseInt(results
						.getAttribute(CAT3.NUM_REC_RETURNED));
				Assert.assertEquals(nRecordsReturned, count, ErrorMessage
						.format(ErrorMessageKeys.INFOSET_ITEM_VALUE,
								"@numberOfRecordsReturned"));
				int nextRecord = Integer.parseInt(results
						.getAttribute(CAT3.NEXT_REC));
				Assert.assertEquals(nextRecord, startIndex + count,
						ErrorMessage.format(
								ErrorMessageKeys.INFOSET_ITEM_VALUE,
								"@nextRecord"));
				break;
			default:
				throw new SkipException("Unrecognized namespace: "
						+ namespaceURI);
			}
		}
	}
	 ...
	 *      >Developer best practices guide</a>
	 */
	@Test(description = "OpenSearchDescription: Query element")
	public void executeExampleQueries() {
		List<Node> exampleQueryList = ServiceMetadataUtils
				.getOpenSearchQueriesByRole(this.openSearchDescr, new QName(
						Namespaces.OSD11, "example"));
		if (exampleQueryList.isEmpty()) {
			throw new SkipException(
					"No example queries found in OpenSearch description.");
		}
		for (Node query : exampleQueryList) {
			Map<QName, String> params = OpenSearchTemplateUtils
					.getQueryParameters(query);
			// Assume all params are allowed in template
			List<Node> qryTemplates = OpenSearchTemplateUtils
					.filterURLTemplatesByParam(this.templates, params.keySet()
							.iterator().next());
			for (Node template : qryTemplates) {
				...
				Assert.assertTrue(records.getLength() > 0,
						ErrorMessage.format(ErrorMessageKeys.EMPTY_RESULT_SET,
								Records.getRecordName(qryTemplate
										.getAttribute("type"))));
			}
		}
	}
	 ...
	 *
	 * @param qryTemplate
	 *            An Element representing an OpenSearch query template
	 *            (osd:Url).
	 * @param parameters
	 *            A Map containing the actual query parameters.
