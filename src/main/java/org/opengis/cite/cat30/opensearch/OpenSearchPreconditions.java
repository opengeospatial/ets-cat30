...
public class OpenSearchPreconditions {
	...
	/**
	 * Checks that the service capabilities document advertises OpenSearch support. The
	 * implementation status of the corresponding conformance class must be set to "TRUE".
	 * The ows:DefaultValue element takes precedence; if it does not appear then the value
	 * of the first occurrence of the ows:Value element will be checked.
	 *
	 * <pre>{@literal
	...
	public void checkOpenSearchImplementationStatus(ITestContext testContext) {
		Document cswCapabilities = (Document) testContext.getSuite()
			.getAttribute(SuiteAttribute.TEST_SUBJECT.getName());
		String xpath = String.format("//ows:Constraint[ends-with(@name,'%s')]/ows:DefaultValue", CAT3.CC_OPEN_SEARCH);
		...
		try {
			...
			if (xdmValue.size() == 0) {
				xpath = String.format("//ows:Constraint[ends-with(@name,'%s')]//ows:Value[1]", CAT3.CC_OPEN_SEARCH);
				xdmValue = XMLUtils.evaluateXPath2(source, xpath, nsMap);
			}
		}
		...
		if (null == openSearchDescr) {
			throw new AssertionError(ErrorMessage.get(ErrorMessageKeys.OPENSEARCH_UNAVAIL));
		}
		testContext.getSuite().setAttribute(SuiteAttribute.OPENSEARCH_DESCR.getName(), openSearchDescr);
	}
