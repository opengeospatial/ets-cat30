 ...
 * service endpoint where the <code>Accept</code> request header expresses a preference
 ...
 *
 * <p>
 * <strong>Note:</strong> None of the media types listed above appear in the IANA
 * <a href="http://www.iana.org/assignments/media-types/media-types.xhtml" target=
 * "_blank">media type registry</a>. Registrations in the standards tree must be approved
 * by the IESG or originate from a recognized standards-related organization (see
 * <a href="http://tools.ietf.org/html/rfc6838#section-3.1" target="_blank">RFC 6838</a>);
 ...
 * <li><a href=
 * "http://www.opensearch.org/Specifications/OpenSearch/1.1#OpenSearch_description_elements"
...
public class OpenSearchDescriptionTests extends CommonFixture {
	...
	/** Constant <code>OPENSEARCH_CONSTRAINT="OpenSearchDescriptionDocument"</code> */
	public static final String OPENSEARCH_CONSTRAINT = "OpenSearchDescriptionDocument";

	/**
	 * Initializes the test fixture by:
	 * <ul>
	 * <li>building a Relax NG schema validator for an OpenSearch description document;
	...
	public void getOpenSearchDescriptionFromCapabilities() {
		...
		if (null == values || values.isEmpty()) {
			throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.NAMED_ITEM_NOT_FOUND, OPENSEARCH_CONSTRAINT));
		}
		URI uri = URI.create(values.iterator().next());
		response = ClientUtils.buildGetRequest(uri, null, MediaType.valueOf(CAT3.APP_VND_OPENSEARCH_XML),
				MediaType.valueOf(CAT3.APP_OPENSEARCH_XML));
		Assert.assertEquals(response.getStatus(), Response.Status.OK.getStatusCode(),
				ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS));
		Assert.assertTrue(XMLUtils.isXML(response.getMediaType()),
