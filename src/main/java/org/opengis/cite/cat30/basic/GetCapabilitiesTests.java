...
/**
 * Provides tests pertaining to the <code>GetCapabilities</code> request. This request
 * implements the abstract <em>getCapabilities</em> operation defined in the OGCWebService
...
public class GetCapabilitiesTests extends CommonFixture {
	...
	private static final String SCHEMATRON_CSW_CAPABILITIES = ROOT_PKG_PATH + "sch/csw-capabilities-3.0.sch";
	...
	/**
	 * Finds the GET method endpoint for the GetCapabilities request in the capabilities
	 * document.
	 */
	@BeforeClass
	public void findServiceEndpoint() {
		this.getCapabilitiesURI = ServiceMetadataUtils.getOperationEndpoint(this.cswCapabilities, CAT3.GET_CAPABILITIES,
				HttpMethod.GET);
	}
	...
	/**
	 * [Test] Verifies that the content of a complete service capabilities document is
	 * schema-valid. All implementations must support the GET method for a GetCapabilities
	 * request.
	 * <p>
	...
	public void getFullCapabilitiesAcceptVersion3() {
		...
		response = ClientUtils.buildGetRequest(this.getCapabilitiesURI, qryParams, MediaType.APPLICATION_XML_TYPE);
	...
	/**
	 * [Test] Attempts to retrieve the capabilities document from the base URL (endpoint
	 * for GetCapabilities via GET). The Accept header indicates that any media type is
	 * acceptable ("&#42;/&#42;"); this is equivalent to omitting the Accept header. The
	 * response shall include a complete XML representation.
	 *
	 * @see "OGC 12-176r6, 6.4: Obtaining service metadata"
	 */
	@Test(description = "Requirements: 006")
	public void getCapabilitiesFromBaseURL() {
		response = ClientUtils.buildGetRequest(this.getCapabilitiesURI, null, MediaType.WILDCARD_TYPE);
		Assert.assertEquals(response.getStatus(), Response.Status.OK.getStatusCode(),
				ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS));
		Source source = ClientUtils.getResponseEntityAsSource(response, null);
	...
	/**
	 * [Test] Attempts to retrieve the capabilities document from the base URL (endpoint
	 * for GetCapabilities via GET). The Accept header indicates XML as the preferred
	 * media type:
	 *
	...
	/**
	 * [Test] Query parameter values must be handled in a case-sensitive manner. The
	 * request specifies <code>request=getCapabilities</code>; an exception report is
	...
	/**
	 * [Test] If the required "service" parameter is missing, an exception report with
	 * status code 400 must be produced. The expected OGC exception code is
	 * {@value org.opengis.cite.cat30.CAT3#MISSING_PARAM_VAL}.
	 *
	 * @see "OGC 12-176r5, Table 5: KVP encoding of common operation request parameters"
	 */
	...
	/**
	 * [Test] Verifies that a request for an unsupported version of a capabilities
	 * document produces an exception report containing the exception code
	 * "VersionNegotiationFailed".
	 *
	 * The status code must be 400 (Bad Request) and the response entity must be an XML
	 * document having {http://www.opengis.net/ows/2.0}ExceptionReport as the document
	 * element.
	 *
	...
	public void getCapabilitiesWithUnsupportedVersion() {
		...
		response = ClientUtils.buildGetRequest(this.getCapabilitiesURI, qryParams, MediaType.APPLICATION_XML_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.VER_NEGOTIATION_FAILED, CAT3.ACCEPT_VERSIONS);
	}

	/**
	 * [Test] Verifies that a request for a known but unsupported representation (format)
	 * of a capabilities document produces an exception report containing the exception
	 ...
	 * element.
	 *
	 * @see "OGC 06-121r9, Table 5: GetCapabilities operation request URL parameters"
	 */
	@Test(description = "Requirements: 036,037,042")
	public void getCapabilitiesInUnsupportedFormat() {
		...
		response = ClientUtils.buildGetRequest(this.getCapabilitiesURI, qryParams, MediaType.WILDCARD_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.INVALID_PARAM_VAL, CAT3.ACCEPT_FORMATS);
	}

	/**
	 * [Test] Verifies that a request for a supported representation (format) of a
	 * capabilities document produces the expected response entity. All supported formats
	 * must be listed in the capabilities document as values of the "AcceptFormats"
	 * parameter. The media type "text/xml" must be supported, but it need not be
	 * explicitly listed.
	 *
	...
	public void getCapabilitiesInSupportedFormat() {
		...
		qryParams.put(CAT3.SERVICE, CAT3.SERVICE_TYPE_CODE);
		qryParams.put(CAT3.ACCEPT_VERSIONS, CAT3.VERSION_3_0_0);
		Set<String> allowedFormats = ServiceMetadataUtils.getParameterValues(cswCapabilities, CAT3.GET_CAPABILITIES,
				CAT3.ACCEPT_FORMATS);
		...
		for (String format : allowedFormats) {
			...
			Assert.assertEquals(mediaType, MediaType.valueOf(format),
					ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_MEDIA_TYPE));
		}
	}

	/**
	 * [Test] Verifies that a request for a part of a capabilities document produces the
	 * expected response entity. All recognized section names must be listed in the
	 * capabilities document as values of the "Sections" parameter; the value "All"
	 * indicates that a complete document is requested.
	 *
	 * <p>
	...
	public void getCapabilitiesBySection() {
		...
		qryParams.put(CAT3.SERVICE, CAT3.SERVICE_TYPE_CODE);
		qryParams.put(CAT3.ACCEPT_VERSIONS, CAT3.VERSION_3_0_0);
		Set<String> sections = ServiceMetadataUtils.getParameterValues(cswCapabilities, CAT3.GET_CAPABILITIES,
				CAT3.SECTIONS);
		sections.add("All");
		for (String section : sections) {
			...
			if (section.equals("All")) {
				...
				try {
					...
					Assert.assertTrue(result, ErrorMessage.format(ErrorMessageKeys.XPATH_ERROR, xpath));
				}
