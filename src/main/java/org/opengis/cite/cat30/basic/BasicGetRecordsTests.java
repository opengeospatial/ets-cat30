...
/**
 * Provides tests that apply to the <code>GetRecords</code> request using the KVP syntax.
 *
 * <p>
 * The KVP syntax must be supported; this encoding is generally used with the GET method
 * but may also be used with the POST method; however a POST method binding is very
 * unusual and will be advertised in the capabilities document as an operational
 * constraint as indicated below. The media type of a KVP request entity is
...
public class BasicGetRecordsTests extends CommonFixture {
	...
	public void findRequestEndpoints(ITestContext testContext) {
		this.getURI = ServiceMetadataUtils.getOperationEndpoint(this.cswCapabilities, CAT3.GET_RECORDS, HttpMethod.GET);
		if (null == this.getURI.getScheme()) {
			throw new SkipException("GET endpoint for GetRecords request not found.");
		}
		this.postURI = ServiceMetadataUtils.getOperationEndpoint(this.cswCapabilities, CAT3.GET_RECORDS,
				HttpMethod.POST);
	}

	/**
	 * Gets information about the sample data obtained from the IUT, including: its total
	 * geographic extent; a collection of record titles; and a set of record identifiers.
	 ...
	 * <code>namespace</code> parameter declares a namespace binding for the common type
	 ...
	 * </p>
	 * <ul>
	 * <li>OGC 12-176r6, Table 19: KVP encoding for GetRecords operation request</li>
	...
	/**
	 * [Test] Submits a GetRecords request with no search criteria. The requested record
	 * representation is csw:SummaryRecord; the csw:SearchResults element in the response
	 * cannot be empty.
	 */
	...
	/**
	 * [Test] Submits a GetRecords request with no search criteria. The Accept header and
	 * the outputFormat parameter express a preference for an Atom feed in the response.
	 * The resulting atom:feed must be valid according to RFC 4287.
	 *
	 * <p style="margin-bottom: 0.5em">
	 * <strong>Sources</strong>
	 * </p>
	 ...
	 * <li>OGC 10-032r8, Table 6: Elements of Search operation response in the atom:feed
	 * element describing the search service</li>
	...
	public void getSummaryRecordsInAtomFeed() {
		...
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.APPLICATION_ATOM_XML_TYPE);
		Assert.assertEquals(response.getStatus(), Response.Status.OK.getStatusCode(),
				ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS));
		Assert.assertEquals(ClientUtils.removeParameters(response.getMediaType()), MediaType.APPLICATION_ATOM_XML_TYPE,
		...
		try {
			...
			Logger.getLogger(GetRecordByIdTests.class.getName())
				.log(Level.WARNING, "Error attempting to validate Atom feed.", ex);
		}
	...
	/**
	 * [Test] Submits a GetRecords request that specifies an unsupported output format
	 * (<code>text/example</code>) as the value of the outputFormat parameter. An
	...
	public void getRecordsInUnsupportedOutputFormat() {
		...
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.WILDCARD_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.INVALID_PARAM_VAL, CAT3.OUTPUT_FORMAT);
	}

	/**
	 * [Test] Submits a GetRecords request that specifies an unknown <code>typeName</code>
	...
	public void getRecordsWithUnknownTypeName() {
		...
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.APPLICATION_XML_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.INVALID_PARAM_VAL, CAT3.TYPE_NAMES);
	}

	/**
	 * [Test] Submits a GetRecords request that specifies an unknown
	 * <code>elementSetName</code> parameter value. An exception report is expected in
	...
	public void unknownElementSetName() {
		...
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.APPLICATION_XML_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.INVALID_PARAM_VAL, CAT3.ELEMENT_SET);
	}

	/**
	 * [Test] Submits a GetRecords request that specifies an unsupported output schema
	 * ("urn:uuid:6a29d2a8-9651-47a6-9b14-f05d2b5644f0"). An exception report is expected
	 * in response with HTTP status code 400 and exception code
	 * "{@value org.opengis.cite.cat30.CAT3#INVALID_PARAM_VAL}".
	 */
	@Test(description = "Requirements: 035,037,042")
	...
	/**
	 * [Test] Submits a GetRecords request where the 'elementName' parameter value
	 * identifies a single element from the output schema (dc:title). The response must be
	 * augmented with additional elements so as to be schema valid. Furthermore, every
	 * record in the result set must contain one or more (required) dc:title elements; no
	 * other optional elements must appear.
	 */
	...
	public void presentTitleProperty() {
		...
		qryParams.put(CAT3.NAMESPACE, String.format("xmlns(tns=%s)", Namespaces.DCMES));
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.APPLICATION_XML_TYPE);
		Assert.assertEquals(response.getStatus(), Response.Status.OK.getStatusCode(),
				ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS));
		Document entity = getResponseEntityAsDocument(response, null);
		...
		Element results = (Element) entity.getElementsByTagNameNS(Namespaces.CSW, CAT3.SEARCH_RESULTS).item(0);
		ETSAssert.assertXPath("not(csw:SummaryRecord[dc:type or dc:subject or dc:format or ows:BoundingBox])", results,
				null);
	}

	/**
	 * [Test] Submits a GetRecords request that specifies an element name not declared in
	 * the output schema. An exception report is expected in response with HTTP status
	...
	public void presentUnknownRecordProperty() {
		...
		response = ClientUtils.buildGetRequest(this.getURI, qryParams, MediaType.APPLICATION_XML_TYPE);
		ETSAssert.assertExceptionReport(response, CAT3.INVALID_PARAM_VAL, CAT3.ELEMENT_NAME);
	}

	/**
	 * [Test] Submits a GetRecords request that contains both the <code>ElementName</code>
	 * and <code>ElementSetName</code> parameters. Since these parameters are mutually
	...
	/**
	 * [Test] Submits a GetRecords request where the <code>startPosition</code> and
	 * <code>maxRecords</code> parameters have non-default values. The csw:SearchResults
	...
	public void getPartialResults() {
		...
		Assert.assertEquals(results.getElementsByTagNameNS(Namespaces.CSW, "SummaryRecord").getLength(), maxRecords,
				ErrorMessage.format(ErrorMessageKeys.RESULT_SET_SIZE, "csw:SummaryRecord"));
		Assert.assertEquals(Integer.parseInt(results.getAttribute(CAT3.NUM_REC_RETURNED)), maxRecords,
				ErrorMessage.format(ErrorMessageKeys.INFOSET_ITEM_VALUE, "@numberOfRecordsReturned"));
		Assert.assertEquals(Integer.parseInt(results.getAttribute(CAT3.NEXT_REC)), startPosition + maxRecords,
				ErrorMessage.format(ErrorMessageKeys.INFOSET_ITEM_VALUE, "@nextRecord"));
	}

	/**
	 * [Test] Submits a GetRecords request where the <code>startPosition</code> and
	 * <code>maxRecords</code> parameters have non-default values. The atom:feed element
	...
	public void getPartialResultsAsAtomFeed() {
		...
		Assert.assertNotNull(feedNode, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "atom:feed"));
		Element feed = (Element) feedNode;
		Assert.assertEquals(feed.getElementsByTagNameNS(Namespaces.ATOM, "entry").getLength(), maxRecords,
		...
		Assert.assertNotNull(startIndex, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "os:startIndex"));
		Assert.assertEquals(Integer.parseInt(startIndex.getTextContent()), startPosition,
		...
		Assert.assertEquals(Integer.parseInt(itemsPerPage.getTextContent()), maxRecords,
