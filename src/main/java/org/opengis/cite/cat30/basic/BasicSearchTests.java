...
/**
 * Includes GetRecords tests pertaining to the <code>Filter-FES-KVP</code> conformance
...
public class BasicSearchTests extends CommonFixture {
	...
	/**
	 * [Test] Submits a GetRecords request with a BBOX parameter that includes an invalid
	 * CRS reference ("urn:ogc:def:crs:EPSG::0000"). An exception report is expected in
	...
	public void getRecordsByBBOXWithUnsupportedCRS() {
		...
		qryParams.put(CAT3.BBOX, "472944,5363287,492722,5455253,urn:ogc:def:crs:EPSG::0000");
	...
	/**
	 * [Test] Submits a GetRecords request with a BBOX parameter that includes a supported
	 * CRS reference. The brief records in the response must all contain an
	 * ows:BoundingBox (or ows:WGS84BoundingBox) element that intersects the specified
	 * bounding box.
	 *
	 * <p style="margin-bottom: 0.5em">
	 * <strong>Sources</strong>
	...
	/**
	 * [Test] Submits a GetRecords request with a 'bbox' parameter that uses the default
	 * CRS ("urn:ogc:def:crs:OGC:1.3:CRS84"). The summary records in the response must all
	 * contain an ows:BoundingBox (or ows:WGS84BoundingBox) element that intersects the
	...
	public void getSummaryRecordsByWGS84BBOX() {
		...
		Envelope bbox = this.geoExtent;
		try {
			if (!bbox.getCoordinateReferenceSystem().equals(CommonCRS.WGS84.normalizedGeographic())) {
				bbox = new GeneralEnvelope(Envelopes.transform(bbox, CommonCRS.WGS84.normalizedGeographic()));
			}
		}
	...
	/**
	 * [Test] Submits a GetRecords request with a 'recordIds' parameter that contains a
	 * (comma-separated) list of two record identifiers. Two matching records
	 * (csw:SummaryRecord) are expected in the response.
	 */
	...
	public void getMultipleRecordsById() {
		...
		for (int i = 0; i < recordList.getLength(); i++) {
			...
			Assert.assertFalse(recIdList.isEmpty(),
					ErrorMessage.format(ErrorMessageKeys.ID_NOT_FOUND, Records.getRecordId(record)));
		}
	}

	/**
	 * [Test] Submits a GetRecords request where the 'q' parameter value is a single
	 * URL-encoded term that occurs in at least one catalog record. The result set must
	 * not be empty.
	 *
	 * <p>
	 * <strong>Note: </strong>According to Table 4 in <em>OGC OpenSearch Geo and Time
	 * Extensions</em> (OGC 10-032r8), the domain of keyword matching should include the
	...
	/**
	 * [Test] Submits a GetRecords request where the <code>q</code> parameter contains a
	 * subject word and the <code>maxRecords</code> parameter value has the value "0"
	...
	public void getRecordsBySubjectWithoutResults() {
		...
		Assert.assertEquals(response.getStatus(), Response.Status.OK.getStatusCode(),
				ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS));
		Document entity = getResponseEntityAsDocument(response, null);
		Element results = (Element) entity.getElementsByTagNameNS(Namespaces.CSW, "SearchResults").item(0);
		Assert.assertNotNull(results, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "csw:SearchResults"));
		...
		Assert.assertEquals(Integer.parseInt(results.getAttribute(CAT3.NUM_REC_RETURNED)), 0,
		...
		Assert.assertTrue(Integer.parseInt(results.getAttribute(CAT3.NUM_REC_MATCHED)) > 0,
				ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "numberOfRecordsMatched > 0"));
	}

	/**
	 * [Test] Submits a GetRecords request where the 'q' parameter value is randomly
	 * generated text. The result set is expected to be empty.
	 */
	@Test(description = "OGC 12-176, Table 6 - Text search")
	...
	/**
	 * [Test] This test submits a GetRecords request where the 'q' parameter value
	 * contains two terms. The terms are separated by a space character, and the parameter
	 * value as a whole is percent-encoded. The result set must not be empty.
	 */
	@Test(description = "OGC 12-176, Table 6 - Text search")
	 ...
	 * <code>q</code> parameters, where the latter matches one or more record titles. The
	...
	public void getRecordsByBBOXAndTitle() {
		...
		try { // get titles for records with bbox
			XdmValue titles = this.datasetInfo
				.findItems("//csw:Record[ows:BoundingBox or ows:WGS84BoundingBox]/dc:title", null);
		...
		Assert.assertTrue(Integer.parseInt(numReturned) <= maxRecords,
				ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "numberOfRecordsReturned <= maxRecords"));
