...
<sch:schema id="exception-report" 
  xmlns:sch="http://purl.oclc.org/dsdl/schematron" 
  xml:lang="en"
  queryBinding="xslt2"
  see="http://portal.opengeospatial.org/files/?artifact_id=38867">

  <sch:title>ISO Schematron schema for CSW 3.0 service exception reports.</sch:title>

  <sch:ns prefix="ows" uri="http://www.opengis.net/ows/2.0" />

  <sch:p>This schema specifies constraints concerning the structure and content 
  of OGC service exception reports as defined in OGC 06-121r9, cl. 8 (Exception 
  Reports).</sch:p>
  ...
  <sch:let name="version" value="'3.0.0'" />

  <sch:pattern id="ExceptionReportPattern">
    <sch:title>Rules for OGC exception reports.</sch:title>
    <sch:rule context="/">    
      <sch:assert test="ows:ExceptionReport" 
        diagnostics="msg.root.en">
	Expected exception report with [local name] = "ExceptionReport" and [namespace name] = "http://www.opengis.net/ows/2.0".
      </sch:assert>
      <sch:assert test="//ows:ExceptionReport/@version = $version" 
        diagnostics="msg.version.en">
	Expected @version = '<sch:value-of select="$version"/>' (latest CAT 3.0 schema).
      </sch:assert>
    </sch:rule>
  ...
  <sch:pattern id="MissingParameterValuePattern">
	...
	The @locator attribute should give the name of the missing parameter.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern id="InvalidParameterValuePattern">
	...
	The @exceptionCode attribute must have the value 'InvalidParameterValue'.
      ...
      <sch:assert test="string-length(ows:Exception/@locator) > 0">
	The @locator attribute should give the name of the invalid parameter.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
